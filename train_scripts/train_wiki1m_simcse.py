...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/media/yueyulin/bigdata/data/wiki1m/', help='Path to the training data')
    ...
    wiki_folder = args.data_path

    ## Now we need a list of train sentences.
    ## In this example we simply use all sentences that don't appear in the train/dev set
    train_sentences = []
    with open(os.path.join(wiki_folder, 'wiki1m_for_simcse.txt'), 'r', encoding='utf-8') as fIn:
        for line in fIn:
            line = line.strip()
            train_sentences.append([line,line])
    ...
    rwkv_args.wandb = 'wiki_simcse'
    rwkv_args.run_name = 'wiki_simcse_trainer'

...
class RWKV_Tmix_x060(MyModule):
    ...
    def forward(self, x,mask=None):
        #mask is a tensor [B,T] with 0s where padding is present
        if mask is None:
            #mask is not provided, assume no padding
        ...
        rev_x = RUN_CUDA_RWKV6(B, T, C, H, r, reverse_x(k,rev_idx), reverse_x(v,rev_idx), w, u=self.time_faaaa)
        x = x + reverse_x(rev_x,rev_idx)
    ...
    def forward(self, x, x_emb=None,mask=None):
        ...
        if self.args.dropout == 0:
            ...
            if self.layer_id == 0 and args.pre_ffn > 0:
                ...
                x = self.drop0(x + self.att(self.ln1(x),mask=mask))
    ...
    def forward(self, idx):
        ...
        if args.tiny_att_dim > 0:
            for block in self.rwkvModel.blocks:
                if args.grad_cp == 1:
                    x = deepspeed.checkpointing.checkpoint(block, x, x_emb,mask)
                else:
                    x = block(x, x_emb,mask)
        else:
            for block in self.rwkvModel.blocks:
                if args.grad_cp == 1:
                    x = deepspeed.checkpointing.checkpoint(block, x,mask=mask)
                else:
                    x = block(x,mask=mask)
...
class RwkvForSequenceEmbedding(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx <= args.skip_steps:
            return torch.tensor(0.0,requires_grad=True,device=query.device)
        if "negative" in batch:
            negative = batch["negative"]
        else:
            negative = None
